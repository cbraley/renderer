#ifndef NO_DYN_SPECTRUM_H
#define NO_DYN_SPECTRUM_H
...
public:
     ...
     */
    NoDynSpectrum(CIEReferenceIlluminant illum);

    /**
     *  Construct a spectrum from a spectral matching curve(CMF).
     */
    NoDynSpectrum(SpectralMatchingCurve sCurve);
     ...
     */
    NoDynSpectrum(const std::vector<float>& values,
     ...
     */
    NoDynSpectrum(float* vals, float minNmVal,
     ...
     */
    NoDynSpectrum(float val, float minnm, float maxnm);
     ...
     */
    NoDynSpectrum(float Tkelvin, float minNmVal, float maxNmVal, int numSamples);
    ...
    NoDynSpectrum(spectral_eval_function seval, float minNmVal, float maxNmVal, float stepVal);

    /**
     *  Copy constructor.  Deep copy.
     */
    NoDynSpectrum(const NoDynSpectrum& other);

    /**
     *  Assignment operator.  Deep copy.
     */
    NoDynSpectrum& operator=(const NoDynSpectrum& rhs);

    /**
     *  Get a spectrum from disk. The ok parameter is set to true on success, failse
     *  on failure.
     */
    static NoDynSpectrum fromDisk(const std::string& path, bool& ok);
    ...
    /**
     *  Find the p-norm of a NoDynSpectrum, where the samples are interpreted as a vector.
    ...
    bool operator==(const NoDynSpectrum& other)const;
    bool operator!=(const NoDynSpectrum& other)const;

    //Operators
    //NoDynSpectrum OP scalar operators
    NoDynSpectrum operator*(float scalar)const;
    NoDynSpectrum operator/(float scalar)const;
    NoDynSpectrum operator+(float scalar)const;
    NoDynSpectrum operator-(float scalar)const;

    //NoDynSpectrum OP spectrum
    NoDynSpectrum operator*(const NoDynSpectrum& rhs)const;
    NoDynSpectrum operator/(const NoDynSpectrum& rhs)const;
    NoDynSpectrum operator+(const NoDynSpectrum& rhs)const;
    NoDynSpectrum operator-(const NoDynSpectrum& rhs)const;
    ...
    friend std::ostream& operator<<(std::ostream& os, const NoDynSpectrum& s);
private:
    ...
    static NoDynSpectrum getResultSpectrum(const NoDynSpectrum& lhs, const NoDynSpectrum& rhs);
