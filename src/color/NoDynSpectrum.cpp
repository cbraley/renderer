...
void NoDynSpectrum::assertInvariant()const{
...
NoDynSpectrum::NoDynSpectrum(void) :
...
NoDynSpectrum::NoDynSpectrum(CIEReferenceIlluminant illum){
...
NoDynSpectrum::NoDynSpectrum(SpectralMatchingCurve sCurve) :
    N(-1), minNm(-1.0f),
...
NoDynSpectrum::NoDynSpectrum(const std::vector<float>& values, float firstSampleNm,
...
NoDynSpectrum::NoDynSpectrum(float* vals, float minNmVal, float maxNmVal,
...
NoDynSpectrum::NoDynSpectrum(float val, float minnm, float maxnm) :
...
NoDynSpectrum::NoDynSpectrum(spectral_eval_function seval, float minNmVal,
...
NoDynSpectrum NoDynSpectrum::fromDisk(const std::string& path, bool& ok){
    ...
    NoDynSpectrum ret(dataPtr, minWavelen, maxWavelen, stpWavelen, N);
    delete[] dataPtr;
    ok = done;
    return ret;
}
...
/**
    ...
    */
bool NoDynSpectrum::toDisk(const std::string& path)const{
    ...
    fs << "#NoDynSpectrum file from Colin Braley's Renderer" << std::endl;
...
void NoDynSpectrum::toXYZ(float& x, float& y, float& z, float K)const{

    //Load the XYZ curves
    NoDynSpectrum X(NoDynSpectrum::CIE_X_10_DEG);
    NoDynSpectrum Y(NoDynSpectrum::CIE_Y_10_DEG);
    NoDynSpectrum Z(NoDynSpectrum::CIE_Z_10_DEG);

    //Take the products
    NoDynSpectrum xProd = X * (*this);
    NoDynSpectrum yProd = Y * (*this);
    NoDynSpectrum zProd = Z * (*this);
...
float NoDynSpectrum::getMinNm()const{
    return minNm;
}
float NoDynSpectrum::getMaxNm()const{
    return maxNm;
}


std::vector<float> NoDynSpectrum::getSampleNms()const{
...
int NoDynSpectrum::getNumSamples()const{
    return N;
}

float NoDynSpectrum::getSampleStep()const{
    return stepNm;
}

float NoDynSpectrum::operator[](int idx)const{
    return data[idx];
}

//#define DEBUG_PRINT
float NoDynSpectrum::operator()(float nm)const{
...
void NoDynSpectrum::setSample(int idx, float val){
    Assert(idx >= 0 && idx < N);
    data[idx] = val;
}


bool NoDynSpectrum::operator==(const NoDynSpectrum& other)const{
...
float NoDynSpectrum::norm(int normP)const{
    Assert(normP > 0);
...
//Copy c-tor
NoDynSpectrum::NoDynSpectrum(const NoDynSpectrum& other) :
...
NoDynSpectrum& NoDynSpectrum::operator=(const NoDynSpectrum& rhs){
    N = rhs.N;
    minNm = rhs.minNm;
    maxNm = rhs.maxNm;
    stepNm = rhs.stepNm;
...
//Operators
NoDynSpectrum NoDynSpectrum::operator*(float scalar)const{
    NoDynSpectrum ret(*this);
    for(int i = 0; i < N; i++){
...
NoDynSpectrum NoDynSpectrum::getResultSpectrum(const NoDynSpectrum& lhs, const NoDynSpectrum& rhs){
    NoDynSpectrum ret;
...
NoDynSpectrum NoDynSpectrum::operator*(const NoDynSpectrum& rhs)const{

    NoDynSpectrum ret = getResultSpectrum(*this, rhs);
    float nm = ret.minNm;
    for(int i = 0; i < N; i++){
    ...
    NoDynSpectrum ret(vals, beginMin, endMax, stepNmMin, N);
    delete[] vals;
...
float NoDynSpectrum::integrate()const{
    return defIntegralTrapezoid(getMinNm(), getMaxNm());
}

float NoDynSpectrum::defIntegralTrapezoid(float nmBegin, float nmEnd)const{
...
std::ostream& operator<<(std::ostream& os, const NoDynSpectrum& s){
    os << "NoDynSpectrum: " << std::endl;
    os << "\tN = " << s.N << std::endl;
